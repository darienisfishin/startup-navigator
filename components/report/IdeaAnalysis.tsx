...
export default function IdeaAnalysis({ analysis }: { analysis: IdeaAnalysisType }) {
  return (
    <div className="bg-white rounded-2xl border border-sky-100 shadow-md p-8 sm:p-10 animate-fade-in-up">
      {/* Section header */}
      <div className="mb-8">
        ...
        <div className="h-1 w-24 rounded-full bg-gradient-to-r from-sky-400 to-blue-500 ml-15 mt-2" />
        <p className="text-text-muted text-sm mt-3">AI evaluation of your business concept.</p>
      </div>

      {/* Idea score — prominent */}
      <div className="flex items-center gap-6 p-6 rounded-2xl bg-gradient-to-r from-sky-50 via-blue-50 to-indigo-50 border border-sky-200 mb-8">
        <div className="text-6xl font-bold text-sky-700" style={{ fontFamily: "var(--font-accent)" }}>
          {analysis.ideaScore}
        </div>
        <div>
          <div className="text-base font-bold text-sky-800">Idea Strength Score</div>
          <div className="text-sm text-sky-600">out of 10</div>
          <div className="mt-1">
            {analysis.ideaScore >= 7 ? (
              <span className="inline-flex items-center gap-1 text-xs font-bold text-green-600 bg-green-100 px-2.5 py-0.5 rounded-full">
                ✓ Strong concept
              </span>
            ) : analysis.ideaScore >= 5 ? (
              <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-600 bg-amber-100 px-2.5 py-0.5 rounded-full">
                → Room to improve
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-xs font-bold text-red-600 bg-red-100 px-2.5 py-0.5 rounded-full">
                ! Needs refinement
              </span>
            )}
          </div>
      ...
      {/* Summary */}
      <p className="text-sm text-text leading-relaxed mb-8">{analysis.summary}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-10">
        {[
          ...
          >
            <div className="text-[10px] uppercase tracking-wide font-semibold mb-1 opacity-70">{m.label}</div>
      ...
      <div className="space-y-6">
        ...
        <div className="p-5 rounded-xl bg-sky-50 border border-sky-100">
          <h3 className="text-sm font-bold text-sky-700 mb-3 flex items-center gap-1.5">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <ul className="space-y-2">
            {analysis.recommendations.map((r, i) => (
              <li key={i} className="text-sm text-sky-900 flex items-start gap-2">
                <span className="text-sky-500 mt-0.5 flex-shrink-0">&rarr;</span>
                {r}
              </li>
            ))}
          </ul>
