...
export default function BrandingFeedback({
  ...
  return (
    <div className="bg-white rounded-2xl border border-border shadow-sm p-8">
      ...
      <p className="text-text-muted text-sm mb-6">Honest, constructive feedback on your brand identity.</p>

      {/* Name analysis */}
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          <div className={`px-4 py-2 rounded-xl border ${
            nameAnalysis.overallScore >= 7 ? "bg-green-50 border-green-200" :
            nameAnalysis.overallScore >= 4 ? "bg-accent-50 border-accent-200" :
            nameAnalysis.overallScore > 0 ? "bg-red-50 border-red-200" :
            "bg-gray-50 border-gray-200"
          }`}>
            <span className={`text-2xl font-bold ${
              nameAnalysis.overallScore >= 7 ? "text-green-700" :
              nameAnalysis.overallScore >= 4 ? "text-accent-700" :
              nameAnalysis.overallScore > 0 ? "text-red-700" :
              "text-gray-500"
            }`}>{nameAnalysis.overallScore}</span>
            <span className="text-sm text-text-muted">/10</span>
          </div>
          <div>
            <div className="text-sm font-semibold text-primary-800">Name Score</div>
            <div className="text-xs text-text-muted">
              {nameAnalysis.overallScore >= 7 ? "Strong foundation" :
               nameAnalysis.overallScore >= 4 ? "Needs improvement" :
               nameAnalysis.overallScore > 0 ? "Significant issues" :
               "No name provided"}
            </div>
          </div>
        </div>
        ...
        <p className="text-sm text-text leading-relaxed p-4 rounded-xl bg-surface border border-border">
          {nameAnalysis.feedback}
        </p>

        <div className="mt-4">
          <h3 className="text-sm font-semibold text-primary-800 mb-2">Action Items</h3>
      ...
      {/* Logo analysis with production readiness */}
      {logoAnalysis && (
        <div className="pt-6 border-t border-border">
          <h3 className="text-lg font-bold text-primary-900 mb-2">Logo Analysis</h3>
          ...
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-primary-800 mb-3">Design Quality</h4>
          ...
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-primary-800 mb-3 flex items-center gap-1.5">
          ...
          <p className="text-sm text-text leading-relaxed p-4 rounded-xl bg-surface border border-border mb-4">
          ...
          <div className="mt-4">
            ...
            <ul className="space-y-2.5">
              {logoAnalysis.improvements.map((imp, i) => (
                <li key={i} className="text-sm text-text flex items-start gap-2 p-3 rounded-lg bg-red-50/50 border border-red-100">
